...
{
    /**
     * Opens the homepage, if authentication is needed, will take care of that as
     * well
     */
    public static void open()
    {
        ...
        // Clear cookies to ensure a new session, this is only needed for local
        // debugging
        ...
        if (siteUsername != null && !siteUsername.isBlank())
        {
            ...
            if (Context.get().configuration.preloadAuthentication)
            {
                ...
                // we don't check the first load... second is enough and the page might be
                // broken anyway,
        ...
        {
            // !! Action run takes care of the handling of action and request for XLT, if
            // this
            ...
            // For any asynchronous action, make sure you wait properly to ensure that the
            // request
            // finishes within this action block, otherwise it might not be measured or end
            // up in the
            // next action.
            // Reopen home page to avoid JS security issue with credentials in URL
            Action.run(NAME, () ->
    ...
    /**
     * Check that the homepage is correct. This activity has to be in the action to
     * get the failure properly reported later on in case this verification fails.
     * 
     * Everything possible can be done here of course. But for a load test the
     * balance has to be right. Every check requires cpu and memory, the more
     * checks, the less scale you get out of a single node in terms of concurrent
     * users. Experiment with that.
     */
